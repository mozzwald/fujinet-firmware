...
if [ ! -z "$PC_TARGET" ] ; then
  ...
  # Android cross-compile. The PC build is otherwise unchanged: Android is just
  # another !ESP_PLATFORM host, so it reuses fujinet_pc.cmake wholesale and only
  # needs the toolchain pointed at the NDK.
  CMAKE_EXTRA_ARGS=()
